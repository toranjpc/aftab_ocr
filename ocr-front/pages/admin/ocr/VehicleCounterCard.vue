<template>
  <div>
    <v-card
      ...
      width="110"
      flat
      :disabled="item.total_vehicles === 0"
      :color="
        item.total_vehicles < item.ocr_vehicles ? '#e9b5c859' : '#d8c8f547'
      "
      @click="getTransits()"
      :elevation="hover ? 4 : 2"
    ...
    >
      <v-card-text
        ...
        :data-repetitive="item.bijacs?.[0]?.ocr_matches_count > 1 ? 1 : 0"
      >
        <div class="text-h6">
        ...
        <v-progress-circular
          :value="(item.ocr_vehicles / item.total_vehicles) * 100"
          :color="
            item.total_vehicles === 0
              ? '#d2d2d2'
              : parseInt(item.total_vehicles) < parseInt(item.ocr_vehicles)
              ? 'red'
              : 'purple'
          "
          :class="{
        ...
        >
          {{
            parseInt(item.total_vehicles) < parseInt(item.ocr_vehicles)
              ? 'مازاد'
              : Math.round((item.ocr_vehicles / item.total_vehicles) * 100) +
                '%'
          }}
        </v-progress-circular>
      </v-card-text>
