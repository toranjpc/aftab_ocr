<template>
  <div class="d-flex flex-column">
    <CameraWidget :col="3" :gate="this.gateId" :matchGate="gateId" />


    <DynamicTemplate>
      <template #header-btn>
        <SseBtn :route="`api/sse/ocr-log?gate_number=${gateId}&receiver_id=${gateId}`" />
      </template>
...
export default {
  ...
  created() {
    ...
    this.gateId = this.$route.params.id || 0;

    this.$majra.init({
      hiddenActions,
      mainRoute: { route: `/ocr-log?gate_number=${this.gateId}`, key: 'OcrLog' },
      relations: [],
      fields: fields(this),
      // query: {
      //   receiver_id: this.gateId,
      //   gate_number: this.gateId,
      // },
    })
