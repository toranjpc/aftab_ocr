<template>
  <v-autocomplete
    ...
    @input="$majra.hasChild(field) ? parentChanged($event) : updateField($event)"
    :items="items"
  ...
  >
    <template #append v-if="getFromField('append')">
      <component
        :is="getFromField('append.component')"
        v-bind="getFromField('append.props')"
      />
    </template>
  </v-autocomplete>
</template>
    ...
    items() {
      ...
      if (this.field.rel) {
        ...
        return "withChange" in this.field.rel
          ? this.field.rel.withChange(this.getItemsWithKey(this.field.rel.model), this)
